...
import com.aemerse.mlkit.BitmapInputInfo
import com.aemerse.mlkit.InputInfo
import com.aemerse.mlkit.R
import com.aemerse.mlkit.Utils
import com.aemerse.mlkit.objectdetection.DetectedObjectInfo
import com.aemerse.mlkit.objectdetection.StaticObjectDotView
import com.aemerse.mlkit.productsearch.*
...
class StaticObjectDetectionActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.photo_library_button -> Utils.openImagePicker(
                this
            )
    ...
    private fun detectObjects(imageUri: Uri) {
        ...
        previewCardCarousel?.adapter = PreviewCardAdapter(listOf()) { showSearchResults(it) }
        ...
        detector?.process(image)
            ?.addOnSuccessListener { objects -> onObjectsDetected(
                BitmapInputInfo(
                    inputBitmap!!
                ), objects) }
            ?.addOnFailureListener { onObjectsDetected(
                BitmapInputInfo(
                    inputBitmap!!
                ), listOf()) }
    }
    ...
    private fun onSearchCompleted(detectedObject: DetectedObjectInfo, productList: List<Product>) {
        ...
        previewCardCarousel?.adapter =
            PreviewCardAdapter(searchedObjectMap.values.toList()) { showSearchResults(it) }
        ...
        for (searchedObject in searchedObjectMap.values) {
            ...
            val animatorSet = AnimatorInflater.loadAnimator(this,
